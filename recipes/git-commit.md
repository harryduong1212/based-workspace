...
inputs:
  - name: scope
    type: string
    required: false
    description: Optional commit scope (e.g., "auth", "api"). When omitted, inferred from the staged file paths.
  - name: split
    type: bool
    required: false
    description: When true, split unrelated staged changes into multiple commits. Default false (single commit).

outputs:
  - name: commit_messages
    type: markdown
    description: One or more Conventional Commits messages, plus the exact `git commit` invocations to apply them.
