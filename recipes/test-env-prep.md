...
inputs:
  - name: feature
    type: string
    required: true
    description: Short feature name or path (e.g., "auth/oauth-pkce", "PROJ-412 rate-limiting"). Used to scope the scaffolding.
  - name: spec_path
    type: string
    required: false
    description: Optional path to an SRS / API spec / ticket export the agent can use as the source for fixtures and test names.

outputs:
  - name: artifacts
    type: markdown
    description: List of files created plus a one-page test plan summarizing strategy, fixture locations, and TODO targets.
