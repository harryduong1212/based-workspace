---
id: git-rebase
name: Git Rebase Plan
description: Plan an interactive rebase — propose the pick/squash/reword/drop sequence and the exact commands, with a backup branch first.
audience: tech
version: 0.1.0
status: experimental
cost: low
...
inputs:
  - name: base
    type: string
    required: false
    description: Base reference to rebase onto. Defaults to the merge-base with `main`.
  - name: goal
    type: string
    required: false
    description: What you want out of it (e.g., "squash WIP commits", "reorder docs commit last", "drop the experiment commit").

outputs:
  - name: rebase_plan
    type: markdown
    description: The proposed `git rebase -i` todo-list contents plus the exact commands to run.
