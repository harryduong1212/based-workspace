...
description: Walk through every active merge conflict, explain both sides, and propose a resolution per file — without applying it until the user confirms.
audience: tech
version: 0.1.0
status: experimental
cost: low
...
inputs:
  - name: file
    type: string
    required: false
    description: Optional single file to focus on. When omitted, walks every conflicted file.

outputs:
  - name: resolution_plan
    type: markdown
    description: Per-conflict explanation + recommended resolution + the exact commands to apply.
