...
1. **`infrastructure/core/` n8n image is pulled, not built**: the compose pulls `docker.io/atom8n/n8n:fork`; there is no in-repo build step. The MCP Inspector runs **host-native** via `./scripts/mcp-inspector.sh`. If you need to build either image from source, clone the upstream repos (`harryduong1212/n8n-atom`, `khanh-atom/mcp-inspector-atom8n`) and follow their READMEs.
