...
- **Action:** Trigger the `/custom-generate-feature-spec` workflow based on the user's raw idea.
...
- **Action:** Trigger the `/custom-generate-db-schema` workflow. Save the Output at `.docs/db/`.
...
- **Action (Stream 3A):** Trigger `/custom-generate-api-spec` to finalize the API Contract and save Output at `.api/`.
- **Action (Stream 3B):** Trigger `/custom-generate-sequence-diagram` to map the interaction flow and save Output at `.docs/sequence-diagrams/`. Both streams must execute simultaneously.
...
- **Action:** Trigger the `/custom-prepare-test-environment` workflow. Save the Test Plan at `.docs/tests/`.
