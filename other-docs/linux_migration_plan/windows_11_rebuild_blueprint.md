...
Create `~/.gitconfig-personal` for personal repos:
```ini
[user]
    name = Dương Hoàng Linh
    email = johny.hitwell@gmail.com
```
...
Workspace-scoped `H:\WORKSPACE\mgm\.npmrc`:
```ini
email = linh.hoang.duong@mgm-tp.com
always-auth = true
registry=https://artifacts.mgm-tp.com/artifactory/api/npm/npm-repos/
//artifacts.mgm-tp.com/artifactory/api/npm/npm-repos/:_authToken=<REDACTED_NPM_TOKEN>
```

### 4.5 Maven Configuration

Create `~/.m2/mgm-settings.xml`:
```xml
<settings>
    <servers>
        <server>
            <id>artifactory</id>
            <username>lhduong</username>
            <password><REDACTED_MAVEN_PASSWORD></password>
        </server>
    </servers>
    <mirrors>
        <mirror>
            ...
            <url>https://artifacts.mgm-tp.com/artifactory/maven-repos</url>
        </mirror>
    </mirrors>
    <profiles>
        <profile>
            ...
            <properties>
                <repoId>artifactory</repoId>
                <snapshotRepoUrl>https://artifacts.mgm-tp.com/artifactory/maven-snapshots</snapshotRepoUrl>
                <releaseRepoUrl>https://artifacts.mgm-tp.com/artifactory/maven-releases</releaseRepoUrl>
            </properties>
        </profile>
    </profiles>
    <activeProfiles>
...
Create `H:\WORKSPACE\mgm\.mvn\maven.config`:
```ini
--settings=C:\Users\johny\.m2\mgm-settings.xml
```
...
{
        "auths": {
                "dockerregistry.mgm-tp.com": {
                        "auth": "<REDACTED_DOCKER_AUTH>"
                }
        }
}
```
...
**Extensions** (install all at once):
```powershell
antigravity --install-extension anthropic.claude-code `
  --install-extension henrikdev.ag-quota `
  --install-extension atom8n.n8n-atom-v3 `
  --install-extension atom8n.mcp-inspector-atom `
  ...
  --install-extension meta.pyrefly `
  --install-extension devsense.phptools-vscode `
  --install-extension devsense.intelli-php-vscode `
  --install-extension devsense.composer-php-vscode `
  --install-extension devsense.profiler-php-vscode `
...
# Minimal install — no settings, no extensions, no keybindings found.
```
...
# Java SDKs (auto-downloaded by IntelliJ to ~/.jdks/):
#   - Amazon Corretto 17.0.15
#   - Amazon Corretto 21.0.7 (default, JAVA_HOME)
#   - Oracle JDK 21.0.2
...
[System.Environment]::SetEnvironmentVariable("JAVA_HOME", "C:\Users\johny\.jdks\corretto-21.0.7", "Machine")
...
### System PATH Entries (Machine — dev-relevant)
```
C:\Users\johny\.jdks\corretto-21.0.7\bin
